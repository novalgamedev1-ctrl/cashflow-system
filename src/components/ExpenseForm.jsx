...
// ─── Image preview modal (reusable, same as UserDashboard) ─────────────────────
  ...
  return (
    ...
    >
      ...
      >
        <button
          onClick={onClose}
          className="absolute -top-4 -right-4 z-10 w-9 h-9 bg-white/10 hover:bg-white/20 border border-white/20 rounded-full flex items-center justify-center transition-colors"
        >
        ...
        <p className="text-center text-white/40 text-xs mt-3">Klik di luar untuk menutup</p>
      </motion.div>
    </motion.div>
  )
}
...
export default function ExpenseForm({ onClose, onSubmit }) {
  ...
  const [proofPreview, setProofPreview]   = useState(null) // local object URL
  ...
  const handleImageChange = (e) => {
    ...
    if (!file) return
    setProofImage(file)
    // Create a local preview URL so admin can inspect before submitting
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const expenseData = {
        ...
        proof_image_url: uploadData.path,   // storage path - resolved to URL on read
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {/* Expense Name */}
            <div>
              <label className="block text-sm font-medium text-white mb-2">Expense Name</label>
              <input
                type="text"
                value={formData.name}
            ...
            {/* Amount */}
            <div>
              <label className="block text-sm font-medium text-white mb-2">Amount (Rp)</label>
              <input
                type="number"
            ...
            <div>
              ...
              >
                {CATEGORIES.map((cat) => (
                  <option key={cat} value={cat} className="bg-dark-primary">{cat}</option>
                ))}
              </select>
            </div>
            ...
            {/* Image Upload */}
            <div>
              <label className="block text-sm font-medium text-white mb-2">Proof Image *</label>
              ...
              <AnimatePresence>
                {proofPreview && (
                  <motion.div
                    ...
                    exit={{ opacity: 0 }}
                    className="relative mb-3 rounded-xl overflow-hidden border border-accent/30 group cursor-pointer"
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                      ...
                      <span className="text-white text-sm font-medium">Lihat ukuran penuh</span>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <label className="flex items-center justify-center w-full px-4 py-3 glass rounded-lg cursor-pointer hover:bg-white/10 transition-colors border-2 border-dashed border-accent/30">
                <div className="flex flex-col items-center justify-center gap-2">
                  ...
                  <span className="text-sm text-white">
                    {proofImage ? `Ganti: ${proofImage.name}` : 'Klik untuk upload gambar'}
                  </span>
                </div>
                <input
                  type="file"
      ...
      <AnimatePresence>
        {showPreview && (
          <ImagePreviewModal url={proofPreview} onClose={() => setShowPreview(false)} />
        )}
      </AnimatePresence>
    </>
  )
